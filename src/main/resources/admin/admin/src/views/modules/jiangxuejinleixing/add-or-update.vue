...
export default {
  data() {
    ...
    return {
      ...
      rules: {
        jiangxuejinleixing: [
          {required: true, message: '奖学金类型不能为空', trigger: 'blur'},
        ],
      }
    };
  },
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `jiangxuejinleixing/info/${id}`,
        method: "get"
      }).then(({data}) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jiangxuejinleixing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({data}) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jiangxuejinleixingCrossAddOrUpdateFlag = false;
                  this.parent.search();
